using System;
using System.Collections.Generic;

namespace HandHeldAPI.Models.HandHeld;

public partial class StdInventoryItemMst
{
    public string? ItemCode { get; set; }

    public double? ItMnLevl { get; set; }

    public double? ItMxLevl { get; set; }

    public double? ItMrFact { get; set; }

    public string? ItPhoto { get; set; }

    public string? ItColor { get; set; }

    public double? ItRoLevl { get; set; }

    public double? ItRoQty { get; set; }

    public string? AccCode { get; set; }

    public string? AlAccCd { get; set; }

    public double? ItemWpac { get; set; }

    public double? ItemAcos { get; set; }

    public string? ItRemark { get; set; }

    public string? LeadTime { get; set; }

    public DateTime? ItLdate { get; set; }

    public double? PurTax { get; set; }

    public double? SaleTax { get; set; }

    public string? AccType { get; set; }

    public double? OpnBal { get; set; }

    public double? ItemCost { get; set; }

    public string? ItSsmple { get; set; }

    public string? ItSpecs { get; set; }

    public string? ItCatNo { get; set; }

    public string? ItPrUnit { get; set; }

    public string? ItSrUnit { get; set; }

    public double? ItmPerpkg { get; set; }

    public string? ItCnFact { get; set; }

    public string? ItAlCode { get; set; }

    public string? ItPicSeq { get; set; }

    public string? ItemDesc { get; set; }

    public string? ItmCtgry { get; set; }

    public string? GenerCode { get; set; }

    public string? MatType { get; set; }

    public string? ItemUom { get; set; }

    public string? TemNo { get; set; }

    public string? Make { get; set; }

    public string? OldItCode { get; set; }

    public string? SUom { get; set; }

    public string? SStat { get; set; }

    public double? MSqty { get; set; }

    public string? PUom { get; set; }

    public string? GolfType { get; set; }

    public double? MPqty { get; set; }

    public double? BSprice { get; set; }

    public double? MrpPrice { get; set; }

    public string? SPackType { get; set; }

    public string? PPackType { get; set; }

    public double? SPackVal { get; set; }

    public double? PPackVal { get; set; }

    public string? PicType { get; set; }

    public DateTime? AsOnDt { get; set; }

    public string? ItemLotNo { get; set; }

    public string? Ved { get; set; }

    public string? Sde { get; set; }

    public string? Sos { get; set; }

    public string? Golf { get; set; }

    public string? Xyz { get; set; }

    public string? InvFreq { get; set; }

    public double? PackCost { get; set; }

    public double? OtherCost { get; set; }

    public double? PbvCost { get; set; }

    public float? ItemThick { get; set; }

    public string? ItemCc { get; set; }

    public string? OpenItem { get; set; }

    public string? PTaxStruCode { get; set; }

    public string? STaxStruCode { get; set; }

    public string? SrtCode { get; set; }

    public string? ItmLot { get; set; }

    public double? ItRecMin { get; set; }

    public double? ItRecMax { get; set; }

    public double? ItIssMin { get; set; }

    public double? ItIssMax { get; set; }

    public string? CcType { get; set; }

    public string? ItemClass { get; set; }

    public short? OrderFreqDays { get; set; }

    public short? AvgDailyUsage { get; set; }

    public string? ItemAcno { get; set; }

    public string? ItemAcnoPur { get; set; }

    public string? ItemAcnoSal { get; set; }

    public string? SubUom { get; set; }

    public double? Ratio1 { get; set; }

    public double? Ratio2 { get; set; }

    public double? BasePrice { get; set; }
}
